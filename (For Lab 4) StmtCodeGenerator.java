...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		...
		Unit stmt;
		stmt = j.newGotoStmt(breakTargets.get(nd.getEnclosingLoop()));
		units.add(stmt);
		return null;
	}
	...
	public Void visitWhileStmt(WhileStmt nd) {
		...
		NopStmt label0 = j.newNopStmt(); // label0 = new NOP statement
		NopStmt label1 = j.newNopStmt(); // label1 = new NOP statement
		
		breakTargets.put(nd, label1); // Put (nd, label1) in HashMap for Break Targets
		
		units.add(label0); // emit statement label0
		Value c = ExprCodeGenerator.generate(nd.getExpr(), fcg); // c = generate code for condition
		units.add(j.newIfStmt(j.newEqExpr(c, IntConstant.v(0)), label1)); // emit statement if c == 0 goto label1
		nd.getBody().accept(this); // generate code for body
		units.add(j.newGotoStmt(label0)); // emit statement goto label0
		units.add(label1); // emit statement label1
